...
public class EntityDaoTest {
    ...
    @Test
    @Order(27)
    @DisplayName("CustomerRepository: Find All")
    @Sql({"/insertOrders.sql"})
    @WithMockUser(username = "jai.vicencio@duocuc.cl", roles = "USER")
    void testCustomerRepositoryFindALL() throws Exception {
        assertFalse(customerRepository.findAll().isEmpty());
        assertEquals(2, customerRepository.findAll().size(), "Customer list should have 2 elements from test db");
    }

    @Test
    @Order(28)
    @DisplayName("CustomerRepository: Find By ID")
    @Sql({"/insertOrders.sql"})
    @WithMockUser(username = "jai.vicencio@duocuc.cl", roles = "USER")
    void testCustomerRepositoryFindById() throws Exception {
        Optional<Customer> customer = customerRepository.findById(10L);
        assertTrue(customer.isPresent());
        assertEquals("Jaime",customer.get().getFirstName(), "Customer 10 is called Jaime on test DB");
    }

    @Test
    @Order(29)
    @DisplayName("CustomerRepository: Find By ID, Empty Response")
    @Sql({"/insertOrders.sql"})
    @WithMockUser(username = "jai.vicencio@duocuc.cl", roles = "USER")
    void testCustomerRepositoryFindByIdEmptyResponse() throws Exception {
        Optional<Customer> customer = customerRepository.findById(0L);
        assertFalse(customer.isPresent(), "Customer 0 is not valid on test DB");
    }

    @Test
    @Order(30)
    @DisplayName("CustomerRepository: Find By Email")
    @Sql({"/insertOrders.sql"})
    @WithMockUser(username = "jai.vicencio@duocuc.cl", roles = "USER")
